...
This skill orchestrates 4 parallel agents using Amp's agent system:
...
- Spawns 4 parallel agents with background execution
...
Use Amp's `Task` tool with `agent_type="explore"` for parallel execution.
...
grep -r "import.*stripe\|import.*supabase\|import.*aws" src/ 2>/dev/null | head -50
```
...
# Import patterns
grep -r "^import" src/ 2>/dev/null | head -100
```
