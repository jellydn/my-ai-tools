...
export const DANGEROUS_PATTERNS: DangerousPattern[] = [
	...
	{
		pattern: /git\s+push\s+.*--force(?!-with-lease\b)/i,
		reason: "force push without --force-with-lease",
	},
	{
		pattern: /git\s+push\s+(?:.*\s)?-f(?:\s|$)/i,
		reason: "force push (-f) without lease",
	},
	...
	// Clean untracked files
	{
		...
		reason: "clean -f (removes untracked files)",
	},
	{
		pattern: /git\s+clean\s+.*-[a-z]*d/i,
		reason: "clean -d (removes untracked directories)",
	},
	...
	// Rewrite history commands
	{
		...
		reason: "interactive rebase (can rewrite history)",
	},
	{
	...
	},
	{
		...
		reason: "reflog expire (removes recovery points)",
	},

	// Aggressive garbage collection
	{
		pattern: /git\s+gc\s+.*--prune=now/i,
		reason: "aggressive garbage collection",
	},

	// Checkout force
	{
		pattern: /git\s+checkout\s+.*--force/i,
	...
	// Stash drop/clear
	{
		...
		reason: "stash drop (permanently removes stash)",
	},
	{
		pattern: /git\s+stash\s+clear/i,
		reason: "stash clear (removes all stashes)",
	},

	// Update-ref delete
	{
		pattern: /git\s+update-ref\s+-d/i,
		reason: "update-ref -d (deletes references)",
	},

	// Replace
	{
		pattern: /git\s+replace/i,
		reason: "replace (creates replacement objects)",
	},
];
