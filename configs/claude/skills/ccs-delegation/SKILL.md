...
description: >-
  ...
  auto-selects optimal profile (glm/kimi/custom) from ~/.ccs/config.yaml,
...
**Profile Selection:**
- Auto-select from `~/.ccs/config.yaml` via task analysis (shows active profile only)
...
2. **Discover active profile**
   - Read `~/.ccs/config.yaml` using Read tool
   - Extract active profile from YAML config
   - If file missing → Error: "CCS not configured. Run: ccs doctor"
   - If no active profile → Error: "No active profile in config.yaml"
...
4. **Select profile**
   - Use active profile from ccs.yml by default
   - If override flag provided: use specified profile
   - If no active profile and no override: Error
...
activeProfile = config.active_profile  # from ~/.ccs/config.yaml

# Check for override flag in task
if (task.match(/--(\w+)/)):
  selected = match[1]
  log("Using override profile: {selected}")
