...
- When using bash commands for file/content search, prefer `fd` (fdfind) and `rg` (ripgrep) over standard `find` and `grep` for better performance and git-awareness; use them as fallback when graph/MCP tools are unavailable or the target is non-code.
